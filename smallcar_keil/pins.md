...
使用 **TIM2** 两路 PWM 输出，频率 50Hz，脉宽约 1.0ms~2.0ms 对应 0°~180°。
...
- PS2 右摇杆映射到云台角度：
  ...
  - 映射区间（可根据实际舵机行程再调）：  
    - 水平 yaw：`rx ∈ [0,255] → angle ∈ [30°,150°]`  
    - 俯仰 pitch：`ry ∈ [0,255] → angle ∈ [45°,135°]`，上推为抬头
  - 调用：  
...
- 所有模块（STM32、TB6612、PS2 接收、舵机、未来树莓派）**必须共地**，否则控制信号会不稳定。
