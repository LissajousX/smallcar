# PS2 手柄协议与 STM32 接线说明

> 本文从原始的 `ps2.md` 文档抽取并整理了 PS2 与 STM32 的硬件接线、时序与通信流程，方便查阅。
...
## 9. 本工程中的 PS2 键位与控制映射（附录）

在 `smallcar_keil/main.c` 中，PS2 手柄按键/摇杆被映射为具体的小车与云台控制逻辑，核心要点如下（详细表格与实测 Data[] 对照见 `doc/ps2_keymap.md`）：

- 速度挡位：
  - 使用 SELECT 在 60% / 80% / 100% 三档之间循环切换，默认中档 80%。
- 小车运动：
  - R1 / R2 作为前进/后退“油门”；
  - L1 / L2 作为原地左右旋转；
  - 左右方向键与左摇杆左右共同决定差速转弯方向与幅度；
  - 上下方向键与左摇杆上下在当前版本中不参与运动控制，预留扩展。
- 云台控制：
  - 彩色键（方块/圆圈/三角/叉号）用于云台 Yaw/Pitch 步进式微调；
  - 在模拟模式（M1）下，右摇杆 RX/RY 作为云台连续微调输入；
  - START 与（M1 下的）右摇杆按下 R3 用于云台一键回中。

PS2 模式标志：

- M0：`Data[1] = 0x41`，数字模式，仅按键有效；
- M1：`Data[1] = 0x73`，模拟模式，摇杆模拟值有效并启用上述增强控制逻辑.
