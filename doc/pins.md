...
## 6. 电池电压采样（Battery ADC）

使用 ADC1 通道 2，接在 PA2 上，通过电阻分压测量整车电池电压。

| 功能             | STM32 引脚       | 方向     | 说明                              |
|------------------|------------------|----------|-----------------------------------|
| 电池电压采样输入 | PA2 (ADC1_IN2)   | 模拟输入 | 只用于 ADC 采样，不做数字输出     |

初始化与采样逻辑见 `battery.c` 中的 `Battery_ADC_Init()` 和 `Battery_GetVoltage_mV()`。
...
| 功能     | STM32 引脚 | 方向       | 说明                                        |
|----------|-----------|------------|---------------------------------------------|
| 蜂鸣器输出 | PA8       | 推挽输出   | 驱动有源蜂鸣器控制脚，高电平响、低电平静音 |

引脚初始化在 `buzzer.c` 的 `Buzzer_Init()` 中完成，具体蜂鸣模式（上电自检、PS2 状态、低电量提示等）由 `Buzzer_Task()` 的状态机实现。
